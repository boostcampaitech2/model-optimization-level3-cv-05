...
class MV2BlockGenerator(GeneratorAbstract):
    ...
    def __call__(self, repeat: int = 1):
        ...
        repeat(=n), [inp, oup, stride, expansion] //
        note original notation from paper is [inp, oup, stride, expansion]
        """
        module = []
        _, s, exp = self.args  
...
class MobileViTBlockGenerator(GeneratorAbstract):
    ...
    def __call__(self, repeat: int = 1):
        ...
        InvertedResidualv3 args consists,
        repeat(=n), [inp, oup, stride, expansion] //
        note original notation from paper is [dim, depth, channel, kernel_size, patch_size, mlp_dim, dropout]
        """
        module = []
        dim, depth, _, kernel_size, patch_size, mlp_dim, dropout = self.args  # c is equivalent as self.out_channel
        inp, oup = self.in_channel, self.out_channel
        for i in range(repeat):
            module.append(
                self.base_module(
